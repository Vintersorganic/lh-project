// Note: Ideally, separate API logic and state management for cleaner, more maintainable code.
...
export default function UserCreationModalForm({ open, handleClose }: UserCreationModalFormProps) {

    const showSnackbar = useSnackbarStore((state) => state.showSnackbar);

    const { register, handleSubmit, reset, formState: { errors, isSubmitting }, setFocus } = useForm<FormFields>({
        mode: 'onBlur',
    ...
    const { mutate } = useMutation({
        ...
        onSuccess: (data) => {
            showSnackbar(`User ${data.first_name} ${data.last_name} created successfully.`, 'success');
            // It would be good to redirect the user to users/[id] to show the newly created user.
            handleClose();
            reset(); // Reset form fields
        },
        onError: (error: any) => {
            showSnackbar(`Failed to create user: ${error?.message || "Unknown error"}`, 'error');
        },
    });
