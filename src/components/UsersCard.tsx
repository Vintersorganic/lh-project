...
import { Avatar, Card, CardContent, Typography, Button, Box, Fade } from '@mui/material';
...
export default function UsersCard() {
    ...
    return (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', mt: 4 }}>
            <Link href={handleUserChangeUrl('previous')} passHref>
                <Button component="a" sx={{ mr: 2, color: 'primary.main' }}><ArrowBackIosIcon /></Button>
            </Link>
            <Fade in={true} timeout={500}>
                <Card raised sx={{ width: 360, textAlign: 'center' }}>
                    <Box sx={{ position: 'relative', height: 150, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <Link href={handleUserChangeUrl('next')} passHref>
                <Button component="a" sx={{ ml: 2, color: 'primary.main' }}><ArrowForwardIosIcon /></Button>
            </Link>
        </Box>
    );
}
